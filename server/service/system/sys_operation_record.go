...
func (operationRecordService *OperationRecordService) GetSysOperationRecordInfoList(info systemReq.SysOperationRecordSearch) (list interface{}, total int64, err error) {
	...
	if info.UserName != "" {
		...
		global.GVA_DB.Model(&system.SysUser{}).Select([]string{"ID"}).Where("userName LIKE ?", "%"+info.UserName+"%").Find(&userList)
		var uIds []uint
...
func (operationRecordService *OperationRecordService) GetUserOperationRecordInfoList(info systemReq.UserOperationRecordSearch) (list interface{}, total int64, err error) {
	limit := info.PageSize
	offset := info.PageSize * (info.Page - 1)
	// 创建db
	db := global.GVA_DB.Model(&system.SysOperationRecord{})
